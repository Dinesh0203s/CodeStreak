...
const AdminDashboard = () => {
    ...
    try {
      ...
      switch (reportType) {
        ...
        case 'daywise-leetcode':
          exportDaywiseLeetCodeReport();
          break;
        case 'daywise-codechef':
          exportDaywiseCodeChefReport();
          break;
  ...
  const exportDaywiseLeetCodeReport = () => {
    ...
    const collegeName = students[0]?.college || 'College';
    const timestamp = new Date().toISOString().split('T')[0];
    exportToCSV(daywiseData, `Daywise_LeetCode_Report_${collegeName.replace(/\s+/g, '_')}_${timestamp}.csv`);
  };

  const exportDaywiseCodeChefReport = () => {
    // Collect all day-wise data from all students
    const daywiseData: Array<{
      Date: string;
      Student: string;
    ...
    const collegeName = students[0]?.college || 'College';
    const timestamp = new Date().toISOString().split('T')[0];
    exportToCSV(daywiseData, `Daywise_CodeChef_Report_${collegeName.replace(/\s+/g, '_')}_${timestamp}.csv`);
  };

  const exportDaywiseOverallReport = () => {
